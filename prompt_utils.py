...
QUESTION_TYPE_PROMPTS = {
    "time": (
        "사용자 질문은 시간 확인이다. "
        "가장 가까운 샘플 시간을 사용해 '답변: 약 N초'와 '근거: ...'만 작성한다."
    ),
    "video_type": (
        "사용자 질문은 영상 종류 설명이다. "
        "첫 줄은 '답변: ... 영상으로 보입니다.'로 쓴다. "
        "둘째 줄은 '근거: ...'로 쓰고, 화면 제목/장소/차량 움직임/충돌 여부를 요약한다."
    ),
    "object_presence": (
        "질문 유형: 객체 존재 질문\n"
        "답변 형식:\n"
    ...
    "summary": (
        "사용자 질문은 영상 요약이다. "
        "첫 줄은 '요약: ...'로 쓰고, 이후 최대 3개 장면을 시간 순서로 쓴다. "
        "사고 원인은 쓰지 말고 보이는 차량 움직임과 장면 변화만 쓴다."
    ),
    "general": (
        "사용자 질문에 직접 답한다. 첫 줄은 '답변: ...'로 쓰고, 둘째 줄은 '근거: ...'로 쓴다."
    ),
}
...
def build_vllm_payload(
    ...
    frame_context = build_frame_context(question_type, sampled_frames)

    # vLLM에는 최종적으로 하나의 텍스트 프롬프트와 여러 장의 이미지가 함께 전달됩니다.
    # 이 텍스트에는 사용자 요청, 질문 목적, 필요한 경우의 시간표, 공통 안전 규칙이 포함됩니다.
    # 영상 종류/일반 요약 질문에는 긴 시간표를 넣지 않습니다. Qwen이 시간표를 답변으로 복사하는 문제가 있었기 때문입니다.
    composed_prompt = (
        f"사용자 질문: {user_request}\n"
        f"질문 목적: {question_type}\n"
        f"{frame_context}\n"
        f"{type_prompt}\n"
...
def build_frame_context(question_type: str, sampled_frames: list[dict[str, Any]]) -> str:
    """
    질문 유형에 맞는 프레임 시간 정보를 만듭니다.

    시간 질문에는 정확한 초 단위 근거가 필요하므로 전체 시간표를 제공합니다.
    영상 종류/요약 질문에는 긴 시간표를 제공하지 않습니다. 이전 테스트에서 모델이 시간표를 그대로
    답변에 복사해 사용자의 질문을 수행하지 못하는 문제가 확인됐기 때문입니다.
    """
    if question_type == "time":
        ...
        return f"시간 확인용 샘플 프레임 시간표:\n{frame_timeline}"

    return f"샘플 프레임 {len(sampled_frames)}장은 시간 순서대로 제공된다. 이 문장은 답변에 쓰지 않는다."
...
def is_internal_prompt_artifact(line: str) -> bool:
    ...
    if any(phrase in stripped for phrase in artifact_phrases):
        return True
    if re.match(r"^-?\s*프레임\s*#?\d+\s*:", stripped):
        return True
    if re.match(r"^-?\s*프레(미엄|이머|임)\s*#?", stripped):
        return True
    return False
...
def compact_display_answer(answer: str, user_request: str) -> str:
    """
    화면에 보여줄 최종 답변을 짧게 정리합니다.

    vLLM 원문은 job JSON의 `raw` 필드에 그대로 저장됩니다. 이 함수는 원문을 삭제하지 않고,
    사용자가 화면에서 바로 읽을 수 있도록 첫 답변과 핵심 근거 몇 문장만 남깁니다.
    새 사실을 만들지 않기 위해 모델이 이미 출력한 문장 안에서만 줄입니다.
    """
    question_type = classify_question_type(user_request)
    if question_type not in {"video_type", "summary", "general", "incident"}:
        return answer
    ...
    first_line = next((line for line in lines if line.startswith(("답변:", "요약:"))), lines[0] if lines else "")
    supporting_text = " ".join(line for line in lines if line != first_line)

    # 문장 단위로 자른 뒤, 화면에서 긴 문단이 되지 않도록 최대 2개 근거만 남깁니다.
    # 정규식은 한국어 마침표/물음표/느낌표 뒤의 공백을 기준으로 분리합니다.
    sentences = [sentence.strip() for sentence in re.split(r"(?<=[.!?。])\s+", supporting_text) if sentence.strip()]
    selected = []
    for sentence in sentences:
        if is_internal_prompt_artifact(sentence):
            continue
    ...
    if selected:
        ...
        if not evidence.startswith(("근거:", "주요 장면:")):
