...
| 일시 | 환경 | 입력 영상 | 프레임 설정 | 처리 결과 | 실패 원인 | 비고 |
| --- | --- | --- | ---: | --- | --- | --- |
...
| 2026-06-09 | 로컬 OpenCV/API | 1fps 프레임 추출 | 30 | 성공 | - | 3.2초 합성 mp4에서 0/1/2/3초 4장 추출, `/api/config` 기본 `30`, 최대 `120` 확인 |
| 2026-06-09 | 로컬 프롬프트 | 중복 `답변:` 후처리 | - | 성공 | - | `답변: 확인 불가`와 `답변: 교통사고`가 함께 나온 응답을 `답변:` 1개로 정리 확인 |
| 2026-06-09 | 로컬 Docker/vLLM | `https://youtu.be/ReLCSJEN3KI` | 30 | 성공 | - | job `1780983244_d6dce62a`, 기본 요약 질문에서 `확인 불가` 미발생, 사고 원인 추정 표현 미검출 |
| 2026-06-09 | 로컬 Docker/vLLM | `https://youtube.com/shorts/P22fW-GBFI4` | 30 | 성공 | - | job `1780984463_3e3b770f`, 내부 프롬프트 규칙 노출 제거, `확인 불가` 미발생, 사고 단정 충돌 문구 정리 확인 |
| 2026-06-09 | 로컬 후처리 | 사고 판단/원인 판단 분리 | - | 성공 | - | 충돌 장면은 `차량 사고 의심`으로 유지하고, 사고 원인/책임은 단정 불가로 분리 확인 |
| 2026-06-09 | 로컬 OpenCV/API | 구간 프레임 추출 | 30 | 성공 | - | 12초 합성 mp4에서 5초 구간 x 3장 기준 9장 추출, `/api/config` 기본 `segment`, `5초`, `3장` 확인 |
| 2026-06-09 | 로컬 프롬프트/후처리 | 질문 미수행 응답 보정 | 30 | 성공 | - | job `1780986355_f272eda5` 원문 기준, 비시간 질문에서 프레임 시간표 미전달 및 화면 답변 2줄 정리 확인 |
| 2026-06-09 | 로컬 후처리 | 중복 답변/근거 제거 | - | 성공 | - | `답변:`과 `근거:` 본문이 같으면 근거 줄을 제거하고, 다른 근거는 유지되는지 함수 회귀 검증 |
| 2026-06-09 | 로컬 프롬프트/후처리 | 근거 없는 장소 단정 완화 | 30 | 성공 | - | 차량 주행 장면을 `지하철 통로`로 단정한 응답을 직접 근거가 있는 도로/터널 구간 표현으로 완화 |
| 2026-06-09 | 로컬 UI | 추출 프레임 확대 미리보기 | - | 성공 | - | 썸네일 클릭용 버튼, 확대 모달, 닫기 버튼/ESC 처리 추가 및 정적 파일 응답 확인 |
| 2026-06-09 | 로컬 UI/후처리 | VLM 응답 가독성 개선 | 30 | 성공 | - | 답변/근거를 카드형 구조로 렌더링하고, 시간 질문의 반복 프레임 근거를 대표 2개로 축약 |
| 2026-06-09 | 로컬 vLLM/API | context length 초과 대응 | 120 | 성공 | - | 36장 입력에서 `8420 > 8192` 에러 확인 후, 추출 프레임과 vLLM 전송 프레임을 분리하고 대표 30장/재시도 15장 로직 검증 |
| 2026-06-09 | 로컬 vLLM/API | 토큰별 전송 프레임 상한 | 120 | 성공 | - | `max_tokens` 512/768/1024/2048 기준 vLLM 전송 상한 36/32/30/24장 계산 검증 |
| 2026-06-09 | 로컬 vLLM/API | vLLM 500 내부 오류 대응 | 120 | 성공 | - | job `1780990122_7b2f02ae`에서 멀티모달 캐시 500 오류 확인 후, 중복 프레임 제거와 내부 오류 축소 재시도 로직 검증 |
