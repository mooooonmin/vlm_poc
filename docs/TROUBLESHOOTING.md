...
기본 실행 주소 `127.0.0.1`은 이 PC 안에서만 접속됩니다. 다른 사용자가 같은 네트워크에서 접속하려면 앱을 `0.0.0.0`으로 실행합니다.
...
다른 사용자는 `http://<이_PC의_IP주소>:8080`으로 접속합니다.

그래도 접속되지 않으면 아래를 확인합니다.

- 두 PC가 같은 네트워크에 있는지 확인
- Windows 방화벽에서 TCP `8080` 인바운드 허용
- 회사/기관 네트워크에서 장치 간 접속을 차단하는지 확인
- 앱이 실제로 `0.0.0.0`으로 실행됐는지 콘솔 출력 확인
- 테스트 사용자에게 vLLM `8000` 포트가 아니라 FastAPI `8080` 포트를 안내했는지 확인
...
1. 1fps 최대 프레임 수를 `15` 또는 `30`으로 낮춤
