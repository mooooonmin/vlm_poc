...
function formatSamplingSummary(videoInfo) {
  ...
  const duplicate = videoInfo.vllm_duplicate_frame_count ? ` / 중복 ${videoInfo.vllm_duplicate_frame_count}장 제외` : "";
  const retry = videoInfo.vllm_reduce_retry_used
    ? ` / 축소 재시도(${videoInfo.vllm_reduce_retry_reason || "unknown"})`
    : videoInfo.vllm_context_retry_used
      ? " / 한도 초과 재시도"
      : "";
  ...
  const mode = videoInfo.sampling_strategy === "segment_representative_frames"
    ...
    : "1fps";
  return ` / ${mode} ${count}/${limit}장${limited}${duplicate}${reduced}${retry}`;
}
