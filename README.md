...
| 모델 ID | 화면 표시 기준 | 주의 |
| --- | --- | --- |
| `Qwen/Qwen3-VL-2B-Instruct` | 기본 경량 VLM | 현재 PoC 기본 모델 |
| `Qwen/Qwen3-VL-4B-Instruct-FP8` | 4B FP8 후보 | 컨테이너를 해당 모델로 다시 시작해야 사용 가능 |
| `Qwen/Qwen2.5-VL-7B-Instruct-AWQ` | 7B AWQ 후보 | RTX 4070 Ti 12GB에서는 VRAM 부족 가능성 별도 확인 필요 |

화면에서 모델 ID를 바꾸는 것은 vLLM 요청 payload의 `model` 값을 바꾸는 동작입니다. 실행 중인 vLLM 컨테이너가 해당 모델을 로드하지 않았다면 요청은 실패할 수 있습니다.
