...
| `POST` | `/api/tmp/cleanup` | 완료/실패 job, 고아 프레임, 평가 샘플, 검증 샘플, 화면 캡처 임시파일 정리. `dry_run=true`로 삭제 예정만 확인 가능 |
...
| `evaluation_runner.py` | 여러 영상 샘플을 순차 분석하고 `logs/evaluation/{run_id}` 리포트 생성 |
...
- 분석에 사용한 업로드/다운로드 영상은 `tmp/jobs/{job_id}/`, 추출 프레임은 `tmp/frames/`에 저장됩니다. 반복 테스트 후 화면의 `임시파일 정리` 버튼을 누르면 완료/실패 job의 영상, 프레임, job 로그와 `tmp/evaluation_samples`, `tmp/validation`, `tmp/layout_*.png` 같은 테스트 산출물을 삭제합니다.
...
평가 결과는 `logs/evaluation/{run_id}/summary.json`, `summary.md`에 저장됩니다.
테스트 결과 요약은 [docs/TEST_RESULTS.md](docs/TEST_RESULTS.md)에서 관리합니다.
...
화면의 사용 모델 필드는 비활성화되어 있습니다. 현재 PoC는 실행 중인 vLLM 컨테이너에 로드된 모델을 사용하며, 화면에서 모델을 바꾸지는 않습니다.
모델을 바꿔 테스트하려면 vLLM 컨테이너를 종료한 뒤 `MODEL_ID` 환경 변수를 원하는 모델로 설정하고 컨테이너를 다시 시작해야 합니다.
